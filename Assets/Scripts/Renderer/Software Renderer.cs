...
{
    [Header("Settings")] [SerializeField, Range(0, 1920)]
    private int screenWidth;

    [SerializeField, Range(0, 1440)] private int screenHeight;
    [SerializeField] private RawImage rawImage;

    [Header("Lighting")] [SerializeField, Range(0.0f, 1.0f)]
        ...
        Color32 color, Texture2D texture)
    {
        ...
        else
        {
            ...
            var splitVert = new ScreenVertex(minYVert.V2 + (maxYVert.V2 - minYVert.V2) * splitAlpha,
                ...
                minYVert.zDepth + (maxYVert.zDepth - minYVert.zDepth) * splitAlpha,
                minYVert.LightFactor + (maxYVert.LightFactor - minYVert.LightFactor) * splitAlpha);
        ...
        Color32 color, Texture2D texture)
    {
        ...
        for (var y = yStart;
            y < yEnd;
            y++, uvEdgeL += uvEdgeStepL, uvEdgeR += uvEdgeStepR, zEdgeL += zEdgeStepL, zEdgeR += zEdgeStepR,
            lightEdgeL += lightEdgeStepL, lightEdgeR += lightEdgeStepR)
        {
            ...
            for (var x = xStartPixel;
                x < xEndPixel;
                x++, uvCoord += uvScanStep, zValue += zScanStep, lightFactor += lightScanStep)
            {
                ...
                {
                    _bitmap.PutPixel(x, y, litColour.r, litColour.g, litColour.b, litColour.a, zValue);
                }
            }
        }
    }
...
public class Bitmap
{
    ...
    public Bitmap(int screenWidth, int screenHeight)
    {
        ...
        DepthBuffer = new float[screenWidth * screenHeight * 4];
    }
    ...
    public void PutPixel(int x, int y, byte r, byte g, byte b, byte a, float zDepth)
    {
        var index = (x + y * _screenWidth) * 4;

        if (zDepth < DepthBuffer[index])
        {
...
public readonly struct RenderMesh
{
    ...
    public RenderMesh(Mesh mesh, Renderer renderer, bool hasTexture, Texture2D texture2D)
    {
        Mesh = mesh;
        Renderer = renderer;
        Color = new Color32((byte) Random.Range(0, 255), (byte) Random.Range(0, 255), (byte) Random.Range(0, 255), 255);
        HasTexture = hasTexture;
        MainTexture = HasTexture ? texture2D : null;
    }
}
...
public struct PointLight
{
    ...
    public PointLight(Vector3 position, float intensity, Color32 color32, float range)
    {
        Position = position;
        Intensity = intensity;
        Color = color32;
...
public struct Vertex
{
    ...
    public Vertex(Vector4 vertex, Vector2 uvCoord, float depthZ)
    {
        V4 = vertex;
        uv = uvCoord;
        zDepth = depthZ;
...
public struct ScreenVertex
{
    ...
    public ScreenVertex(Vector2 vertex, Vector2 uvCoord, float depthZ, float lightFactor)
    {
        V2 = vertex;
        uv = uvCoord;
        zDepth = depthZ;
